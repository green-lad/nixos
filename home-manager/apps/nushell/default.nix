...
{
  config.services.lorri.enable = true;
  ...
  config.home.packages =
    let
      topiary-nushell = builtins.fetchTree {
        ...
        rev = "82966d745e2315a0b467fc2e0164b83d448ee7b0";
      };
      ...
      topiary = inputs.wrapper-manager.lib.wrapWith pkgs {
        basePackage = pkgs.topiary;
        prependFlags = [ "--merge-configuration" ];
        env = {
          TOPIARY_CONFIG_FILE.value = pkgs.writeText "languages.ncl" ''
            {
              languages = {
                nu = {
                  indent = "  ",
  ...
  config.programs = {
    ...
    nushell =
      let
        ...
        change_escape_command = cmd: "r####${cmd}#### | str replace -a r#''\\\\'''# r#'''#";
        fzf_search_history = str: [
          {
            ...
            cmd =
              let
                ...
                bind = "ctrl-y:execute-silent(${change_escape_command "{}"} | wl-copy)+abort";
              in
              ''
                let choice = (
                  history
        ...
        fzf_command_picker = [
          {
            ...
            cmd =
              let
                preview = "${change_escape_command "{3}"}";
              in
              ''
                let choice = (
      ...
      in
      {
        ...
        extraConfig =
          let
            preview = "${change_escape_command "'{1}\\n---\\n{2}'"}";
          in
          ''
                  use std/dirs
                  alias d = dirs
